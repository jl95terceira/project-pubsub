...
public class TestTypeSwitched {
    ...
    public void setUp() throws Exception {
        var requesterFuture = CompletableFuture.supplyAsync(() -> {
            ...
            return TypedRequesterAdaptersCollection.asStringPostGetRequester(TypedRequester.fromSimpleRpc(Requester.fromIo(ioAsServer)));
        ...
        var responderFuture = CompletableFuture.supplyAsync(() -> {
            ...
            return TypeSwitchedResponderAdaptersCollection.asStringPostGetResponder(TypeSwitchedResponder.fromIo(ioAsClient));
        }, (task) -> new Thread(task).start());
    ...
    public void test() {
        ...
        responder.start().get();
        org.junit.Assert.assertEquals("hello, world", requester.getFunction("hello").apply("world").get());
        var intRequester = requester.adapted((Integer i) -> i.toString(), Boolean::parseBoolean);
        org.junit.Assert.assertEquals(Boolean.FALSE , intRequester.getFunction("answer").apply(100).get());
        org.junit.Assert.assertEquals(Boolean.TRUE  , intRequester.getFunction("answer").apply(42).get());
        org.junit.Assert.assertEquals("bye, world"  , requester.getFunction("bye").apply("world").get());
    }
    @org.junit.Test
